...
def plot_error_bars(ax, train_file, test_file, xlabels, width=0.2, ylabel=True, show_legend=False):
    ...
    # Plot MAE bars
    bars1 = ax.bar(x - 1.5*width, train.loc['MAE'], width, color=mysalmon, edgecolor='k', label='Train MAE')
    bars2 = ax.bar(x - width/2, test.loc['MAE'], width, color=mysalmon, edgecolor='k', hatch='//', label='Test MAE')
    
    # Plot RMSE bars
    bars3 = ax.bar(x + width/2, train.loc['RMSE'], width, color=myteal, edgecolor='k', label='Train RMSE')
    bars4 = ax.bar(x + 1.5*width, test.loc['RMSE'], width, color=myteal, edgecolor='k', hatch='//', label='Test RMSE')
...
def create_shared_yaxis_plots(train_file_1, test_file_1, train_file_2, test_file_2, xlabels, output_file, width=0.2):
    ...
    plot_error_bars(ax1, train_file_2, test_file_2, xlabels, width, show_legend=False)
    ...
    plot_error_bars(ax2, train_file_1, test_file_1, xlabels, width, ylabel=False, show_legend=True)
