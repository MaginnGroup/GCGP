...
r2_train, mapd_train, mae_train, rmse_train = evaluate(Y_Train_plt, Y_Train_Pred_plt)
r2_test, mapd_test, mae_test, rmse_test = evaluate(Y_Test_plt, Y_Test_Pred_plt)



# Save the model summary to a txt file
model_file_name = str(f'model_summary_{code}_{seed}.txt')
with open(model_file_name, 'w') as file:
    val = gpflow.utilities.read_values(model)
    file.write(str(val))
    file.write("\n Property: " + str(code))
