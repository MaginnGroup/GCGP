...
def create_bar_charts(df, flag_value=False):
    ...
    for property in df['Property'].unique():
        ...
        filtered_df = property_df[property_df['Flag'] == flag_value]
        ...
        for i, kernel in enumerate(kernels):
            ...
            if kernel == 'Matern12' and property == 'Tb' and flag_value == False:
                kernel_df.loc[-1] = ['Tb', 5, 'Matern12', False, 0]
            elif kernel == 'RQ' and property == 'Tb' and flag_value == True:
                kernel_df.loc[-1] = ['Tb', 5, 'RQ', True, 0]
            elif kernel == 'Matern12' and property == 'Tc' and flag_value == True:
                print(kernel_df)
                kernel_df.loc[-1] = ['Tc', 2, 'Matern12', True, 0]
                kernel_df = kernel_df.sort_values(by='Model')
        ...
        plt.savefig(f'lml_bar_chart_{property}_{flag_value}.png')
        plt.close()
        count += 1

# Call the function with the desired flag value (True or False)
create_bar_charts(df, flag_value=True)
