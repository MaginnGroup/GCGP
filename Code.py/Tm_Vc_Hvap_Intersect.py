...
def buildGP(X_Train, Y_Train, Y_gc, gpConfig, code, featurenorm, retrain_count):
    ...
    model_mf = build_model_with_bounded_params(X_Train, Y_Train, Y_gc, kernel, low_bound, up_bound, alpha_up_bound,\
                                            init_val1, init_val2, init_val3, useWhiteKernel, \
                                            trainLikelihood, anisotropy, typeMeanFunc)
    mf_method = model_mf[1]
    model = model_mf[0]
    ...
    return model, aux, condition_number, obj_func, opt_success, retrain_count, model_pretrain, mf_method
...
def train_gp(X_Train, Y_Train, Y_gc, gpConfig, code, sc_y, featurenorm, retrain_GP, retrain_count):
    ...
    for i in range(retrain_GP):
        model, aux, condition_number, obj_func, opt_success, retrain_count, model_pretrain, mf_method = \
    ...
    return best_model,best_minimum_loss,best_model_success,best_condition_num,trained_hyperparams,best_model_pretrain,sc_y_scale, mf_method
...
dbPath=""
# Property Code
code='Tm_Hvap_Intersect' # Tm_Vc_Intersect     Tm_Hvap_Intersect
